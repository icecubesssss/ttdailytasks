...
import { Play, Pause, CheckCircle2, Clock, Calendar, Lock, BrainCircuit } from 'lucide-react';
...
function TaskItem({
  ...
  const elapsed = task.status === 'running' ? task.totalTrackedTime + (now.getTime() - (task.lastStartTime || 0)) : task.totalTrackedTime;
    ...
    <div className={`group relative p-4 rounded-3xl border transition-all hover:-translate-y-1 hover:shadow-2xl ${isDark ? (isCompleted ? 'bg-slate-800/50 border-slate-700/50 opacity-70' : 'bg-slate-800/90 backdrop-blur-md border-slate-600 shadow-xl shadow-slate-900/50') : (isCompleted ? 'bg-white/60 border-slate-200/50 opacity-70' : 'bg-white/95 backdrop-blur-md border-slate-200 shadow-xl shadow-slate-200/50')} ${isWorking ? 'ring-2 ring-indigo-500 ring-offset-2 dark:ring-offset-slate-900 border-indigo-500' : ''}`}>
      ...
      <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-100 dark:border-slate-700/50">
        <div className="flex flex-col gap-1 text-[10px] font-bold">
          ...
          <div>
            {isEditingDeadline ? (
              <input type="datetime-local" value={tempDeadline} onChange={(e) => setTempDeadline(e.target.value)} onBlur={handleDeadlineSubmit} autoFocus aria-label="Hạn chót" className={`bg-indigo-500/10 border border-indigo-500/30 rounded px-1 py-0.5 outline-none text-[10px] ${isDark ? 'text-indigo-300' : 'text-indigo-600'}`} style={{ colorScheme: isDark ? 'dark' : 'light' }} />
            ) : (
        ...
        <div className="flex items-center gap-1">
          {!isLocked && !isCompleted && (
            <>
              ...
              <button onClick={() => onComplete(task.id)} aria-label="Hoàn thành task" className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${isDark ? 'bg-slate-700 text-slate-400 hover:bg-emerald-500 hover:text-white' : 'bg-slate-100 text-slate-500 hover:bg-emerald-500 hover:text-white'}`}>
